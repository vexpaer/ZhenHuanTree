...
// data = a function returning the content (actually HTML)
	...
	Vue.component('buyable', {
		...
		<div v-if="tmp[layer].buyables && tmp[layer].buyables[data]!== undefined && tmp[layer].buyables[data].unlocked" style="display: grid">
			...
			v-on:click="if(!interval) (shiftDown && layers[layer].buyables[data].buyMax ? buyMaxBuyable(layer, data) : buyBuyable(layer, data))" :id='"buyable-" + layer + "-" + data' @mousedown="start" @mouseleave="stop" @mouseup="stop" @touchstart="start" @touchend="stop" @touchcancel="stop">
